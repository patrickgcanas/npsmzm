import { prisma } from "@/lib/prisma";
import { csatQuestions } from "@/lib/survey";

function csatPercent(record) {
  const values = csatQuestions
    .map((q) => Number(record[q.id]))
    .filter((v) => Number.isFinite(v) && v > 0);
  if (!values.length) return 0;
  return Math.round((values.filter((v) => v >= 4).length / values.length) * 100);
}

export async function getAllResponseHistory() {
  const records = await prisma.surveyResponse.findMany({
    include: { invite: true },
    orderBy: { createdAt: "asc" },
  });

  return records.map((r) => ({
    id: r.id,
    createdAt: r.createdAt.toISOString(),
    clientName: r.invite.clientName,
    clientCode: r.invite.clientCode || "",
    advisor: r.invite.advisor,
    npsScore: r.npsScore,
    csat: csatPercent(r),
    strengths: r.strengths || "",
    improvements: r.improvements || "",
  }));
}

function mapResponse(record) {
  return {
    id: record.id,
    createdAt: record.createdAt.toISOString(),
    clientName: record.invite.clientName,
    clientEmail: record.invite.clientEmail || "",
    clientCode: record.invite.clientCode || "",
    advisor: record.invite.advisor,
    relationshipNote: record.invite.relationshipNote || "",
    journeyStage: record.journeyStage,
    npsScore: record.npsScore,
    strengths: record.strengths,
    improvements: record.improvements,
    otherComments: record.otherComments,
    token: record.invite.token,
    csatAnswers: Object.fromEntries(
      csatQuestions.map((question) => [question.id, Number(record[question.id])])
    ),
  };
}

export async function getDashboardResponses() {
  const records = await prisma.surveyResponse.findMany({
    where: {
      invite: { deletedAt: null },
    },
    include: {
      invite: true,
    },
    orderBy: {
      createdAt: "desc",
    },
  });

  return records.map(mapResponse);
}

export async function getHomeSummary() {
  const responses = await getDashboardResponses();
  return responses;
}

export async function getAllInvites() {
  const invites = await prisma.surveyInvite.findMany({
    where: { deletedAt: null },
    include: { response: true },
    orderBy: { createdAt: "desc" },
  });

  // Load send history separately — table may not exist yet before migration runs
  let sendsMap = {};
  try {
    const allSends = await prisma.surveyInviteSend.findMany({
      orderBy: { sendNumber: "asc" },
    });
    allSends.forEach((s) => {
      if (!sendsMap[s.inviteId]) sendsMap[s.inviteId] = [];
      sendsMap[s.inviteId].push(s);
    });
  } catch {
    // Table not yet created — sends will be empty until migration runs
  }

  return invites.map((invite) => ({
    id: invite.id,
    token: invite.token,
    clientName: invite.clientName,
    clientEmail: invite.clientEmail || "",
    clientCode: invite.clientCode || "",
    advisor: invite.advisor,
    createdAt: invite.createdAt.toISOString(),
    responded: invite.response !== null,
    respondedAt: invite.response?.createdAt?.toISOString() || null,
    viewedAt: invite.viewedAt?.toISOString() || null,
    startedAt: invite.startedAt?.toISOString() || null,
    sentAt: invite.sentAt?.toISOString() || null,
    sendStatus: invite.sendStatus || "pending",
    sendError: invite.sendError || null,
    contractDate: invite.contractDate?.toISOString() || null,
    sends: (sendsMap[invite.id] || []).map((s) => ({
      sendNumber: s.sendNumber,
      sentAt: s.sentAt.toISOString(),
      viewedAt: s.viewedAt?.toISOString() || null,
      startedAt: s.startedAt?.toISOString() || null,
    })),
  }));
}

export async function getInviteByToken(token) {
  return prisma.surveyInvite.findUnique({
    where: { token },
    include: {
      response: true,
    },
  });
}

export function getAppUrl() {
  if (process.env.APP_URL) {
    return process.env.APP_URL.replace(/\/$/, "");
  }

  // VERCEL_PROJECT_PRODUCTION_URL is the stable production domain (no deployment hash)
  // VERCEL_URL is deployment-specific and may trigger Vercel auth protection
  if (process.env.VERCEL_PROJECT_PRODUCTION_URL) {
    return `https://${process.env.VERCEL_PROJECT_PRODUCTION_URL}`;
  }

  if (process.env.VERCEL_URL) {
    return `https://${process.env.VERCEL_URL}`;
  }

  return "http://localhost:3000";
}
