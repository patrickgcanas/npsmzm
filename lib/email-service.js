...
export async function sendInviteEmail(invite, requestOrigin) {
  ...
  try {
    await resend.emails.send({
      ...
      replyTo: "ariane.siciliano@mzmwealth.com",
      to: invite.clientEmail,
      subject: INVITE_SUBJECT,
      html,
    });

    const sendCount = await prisma.surveyInviteSend.count({ where: { inviteId: invite.id } });
    await prisma.$transaction([
      prisma.surveyInvite.update({
        where: { id: invite.id },
        data: { sentAt: new Date(), sendStatus: "sent", sendError: null },
      }),
      prisma.surveyInviteSend.create({
        data: { inviteId: invite.id, sendNumber: sendCount + 1 },
      }),
    ]);
