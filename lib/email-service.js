...
function buildEmailHtml(clientName, inviteUrl, requestOrigin) {
  ...
  const bodyHtml = bodyText
    .split("\n")
    .map((line) => `<p style="margin:0 0 8px 0;font-family:Arial,sans-serif;font-size:15px;color:#1a1a1a;">${line || "&nbsp;"}</p>`)
    .join("");

  const signature = `
    <br/>
    <table cellpadding="0" cellspacing="0" border="0" style="width:560px;border-radius:6px;overflow:hidden;font-family:Arial,sans-serif;">
      <tr>
        <td width="165" style="background-color:#ccd5d3;padding:20px;text-align:center;vertical-align:middle;">
          <img src="${logoUrl}" alt="MZM Wealth" width="90" style="display:block;margin:0 auto;" />
        </td>
        <td style="background-color:#1d3d33;padding:20px 26px;vertical-align:middle;">
          <p style="margin:0 0 10px 0;font-family:Arial,sans-serif;font-size:16px;font-weight:bold;color:#ffffff;letter-spacing:1.5px;text-transform:uppercase;">ARIANE SICILIANO</p>
          <p style="margin:0 0 4px 0;font-family:Arial,sans-serif;font-size:13px;color:#ffffff;">11 97379-5858</p>
          <p style="margin:0 0 10px 0;font-family:Arial,sans-serif;font-size:13px;font-weight:bold;color:#ffffff;">ariane.siciliano@mzmwealth.com</p>
          <p style="margin:0;font-family:Arial,sans-serif;font-size:11px;color:#adc4be;line-height:1.6;">Av. Brig. Faria Lima, 1188 - cj. 131<br/>Pinheiros - CEP: 01451-001</p>
        </td>
      </tr>
    </table>`;

  return `<!DOCTYPE html><html><body style="margin:0;padding:24px;">${bodyHtml}${signature}</body></html>`;
}
