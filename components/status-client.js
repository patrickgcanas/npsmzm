...
export function StatusClient({ initialInvites, appUrl }) {
  ...
  const pendingWithEmail = useMemo(
    () => filtered.filter((i) => !i.responded && i.clientEmail),
    [filtered]
  );
  ...
  function openAllEmails() {
    ...
    pendingWithEmail.forEach((invite, index) => {
      setTimeout(() => {
        window.open(buildMailto(invite, appUrl), "_blank");
        setOpenedCount(index + 1);
        if (index === pendingWithEmail.length - 1) setOpening(false);
      }, index * 600);
    });
  }
        ...
        <div className="panel-header">
          ...
          <div className="panel-header-actions">
            {pendingWithEmail.length > 0 && (
              ...
              >
                {opening
                  ? `Abrindo ${openedCount}/${pendingWithEmail.length}...`
                  : `Abrir ${pendingWithEmail.length} e-mail${pendingWithEmail.length !== 1 ? "s" : ""} no Outlook`}
              </button>
            )}
